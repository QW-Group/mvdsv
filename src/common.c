...
coorddata
MSG_ToCoord(float f,
            int bytes) // return value should be treated as (char*)&ret;
{
  coorddata r;
  switch (bytes) {
  case 2:
...
void SZ_InitEx(sizebuf_t *buf, byte *data, const int length,
               qbool allowoverflow) {
...
void *SZ_GetSpace(sizebuf_t *buf, const int length) {
  ...
  if (buf->cursize + length > buf->maxsize) {
    ...
    if (length > buf->maxsize)
      Sys_Error("SZ_GetSpace: %d/%d is > full buffer size", length,
                buf->maxsize);
...
// WARNING:		non standard behavior for Info_* function, this function
may return NULL while other functions always return at least ""
...
char *Info_KeyNameForKeyNum (char *s, int key)
{
        ...
        while (1)
        {
                ...
                while (*s != '\\')
                {
                        if (!*s || o >= pkey[keyindex] + sizeof(pkey[keyindex])
- 1) return NULL; *o++ = *s++;
                }
                *o = 0;
                s++;
...
void Info_SetValueForStarKey(char *s, const char *key, const char *value,
  ...
  if (*(v = Info_ValueForKey(s, key))) {
    ...
    if (strlen(value) - strlen(v) + strlen(s) >= maxsize) {
      Con_Printf("Info string length exceeded (change: key = '%s', old value = "
                 "'%s', new value = '%s', strlen info = '%d', maxsize = "
                 "'%d')\nFull info string: '%s'\n",
  ...
  if (strlen(_new) + strlen(s) >= maxsize) {
    Con_Printf("Info string length exceeded (add: key = '%s', value = '%s', "
               "strlen info = '%d', maxsize = '%d')\nFull info string: '%s'\n",
...
qbool Info_SetStar(ctxinfo_t *ctx, const char *name, const char *value) {
  ...
  if (strchr(name, '\n') || strchr(value, '\n')) // bad for print functions
    return false;
  if (strchr(name, '$') ||
      strchr(value, '$')) // variable expansion may be exploited, escaping this
    return false;
  if (strchr(name, ';') ||
      strchr(value, ';')) // interpreter may be haxed, escaping this
    return false;
...
#else
  {
    ...
    for (i = 0; value[i]; i++) // len of 'value' should be less than
                               // MAX_KEY_STRING according to above checks
    {
...
qbool Info_Convert(ctxinfo_t *ctx, char *str) {
  ...
  for (; str && str[0];) {
    ...
    strlcpy(value, start + 1,
            str ? min(str - start, (int)sizeof(value)) : (int)sizeof(value));
...
static void Q_normalizetext_Init(void) {
  ...
  // dot
  q_normalize_chartbl[5] = q_normalize_chartbl[14] = q_normalize_chartbl[15] =
      q_normalize_chartbl[28] = q_normalize_chartbl[46] = '.';
  q_normalize_chartbl[5 + 128] = q_normalize_chartbl[14 + 128] =
      q_normalize_chartbl[15 + 128] = q_normalize_chartbl[28 + 128] =
          q_normalize_chartbl[46 + 128] = '.';
  ...
  q_normalize_chartbl[29] = q_normalize_chartbl[29 + 128] =
      q_normalize_chartbl[128] = '(';
  q_normalize_chartbl[31] = q_normalize_chartbl[31 + 128] =
      q_normalize_chartbl[130] = ')';
  ...
  // '='
  q_normalize_chartbl[30] = q_normalize_chartbl[129] =
      q_normalize_chartbl[30 + 128] = '=';
