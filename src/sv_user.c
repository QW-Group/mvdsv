...
// Record voicechat into mvds. Requires player support. 0=noone, 1=everyone,
// 2=spectators only.
...
static void SV_DebugClientCommand(byte playernum, const usercmd_t *cmd,
                                  int dropnum_);
...
static qbool IsInetIP(netadr_t a) { return a.ip[0] != 127 && !IsLocalIP(a); }

//
// pitch clamping
//
// All this OnChange code is because we want the cvar names to have sv_
// prefixes, but don't want them in serverinfo (save a couple of bytes of space)
...
static void OnChange_sv_maxpitch(cvar_t *var, char *str, qbool *cancel) {
  ...
  *cancel = true;
  ...
  newstr = (newval == 80.0f)
               ? (char *)""
               : var->string; // don't show default values in serverinfo
...
qbool PlayerCheckPing(void) {
  ...
  if (sv_client->maxping_met)
    return true;

  int maxping = Q_atof(sv_maxping.string);
  int playerping =
      sv_client->frames[sv_client->netchan.incoming_acknowledged & UPDATE_MASK]
          .ping_time *
      1000;

  if (maxping && playerping > maxping) {
    SV_ClientPrintf(sv_client, PRINT_HIGH,
                    "\nYour ping is too high for this server!  Maximum ping is "
                    "set to %i, your ping is %i.\nForcing spectator.\n\n",
                    maxping, playerping);
    return false;
  }
...
static void Cmd_New_f(void) {
  ...
  if (sv_client->state == cs_preconnected && !sv_client->realip.ip[0] &&
      (int)sv_getrealip.value) {
    char *server_ip = sv_serverip.string[0]
                          ? sv_serverip.string
                          : NET_AdrToString(net_local_sv_ipadr);

    if (!((IsLocalIP(net_local_sv_ipadr) &&
           IsLocalIP(sv_client->netchan.remote_address)) ||
          (IsInetIP(net_local_sv_ipadr) &&
           IsInetIP(sv_client->netchan.remote_address))) &&
        sv_client->netchan.remote_address.ip[0] != 127 &&
        !sv_serverip.string[0]) {
      Sys_Printf("WARNING: Incorrect server ip address: %s\n"
                 "Set hostname in your operation system or set correctly "
                 "sv_serverip cvar.\n",
                 server_ip);
      *(int *)&sv_client->realip = *(int *)&sv_client->netchan.remote_address;
      ...
      if (SV_ClientConnectedTime(sv_client) > 3 ||
          sv_client->realip_count > 10) {
  ...
  if (!sv_client->rip_vip) {
    if (sv_client->state == cs_preconnected) {
      ...
      Info_SetStar(&sv_client->_userinfo_ctx_, "*VIP",
                   sv_client->vip ? va("%d", sv_client->vip) : "");
  ...
  if (msg_coordsize > 2 &&
      !(sv_client->fteprotocolextensions & FTE_PEXT_FLOATCOORDS)) {
  ...
  if (sv.max_edicts > 512 &&
      !(sv_client->fteprotocolextensions & FTE_PEXT_ENTITYDBL)) {
    SV_ClientPrintf(sv_client, 2,
                    "\n\nWARNING:\n"
  ...
  if (sv_client->fteprotocolextensions2) // let the client know
  {
    ...
    MSG_WriteLong(&sv_client->netchan.message,
  ...
  if (sv_client->rip_vip)
    ...
    MSG_WriteString(&sv_client->netchan.message,
                    PR_GetEntityString(sv.edicts->v->message));

  // send the movevars
  MSG_WriteFloat(&sv_client->netchan.message, movevars.gravity);
  MSG_WriteFloat(&sv_client->netchan.message, movevars.stopspeed);
  MSG_WriteFloat(&sv_client->netchan.message,
                 /* sv_client->maxspeed */ movevars
                     .maxspeed); // FIXME: this does't work, Tonik?
  ...
  MSG_WriteFloat(&sv_client->netchan.message,
                 /* sv_client->entgravity */ movevars
                     .entgravity); // FIXME: this does't work, Tonik?
...
static void Cmd_Soundlist_f(void) {
  ...
  for (s = sv.sound_precache + 1 + n;
       *s && sv_client->netchan.message.cursize < (MAX_MSGLEN / 2); s++, n++)
...
static char *TrimModelName(const char *full) {
  ...
  if (len > 4 && !strcmp(shortn + len - 4, ".mdl") &&
      strchr(shortn, '.') == shortn + len - 4) { // strip .mdl
    shortn[len - 4] = '\0';
  }
...
static void Cmd_Modellist_f(void) {
  ...
  for (i = 1 + n; i < maxclientsupportedmodels && *s &&
                  (((i - 1) & 255) == 0 ||
                   sv_client->netchan.message.cursize < (MAX_MSGLEN / 2));
       i++, s++) {
    MSG_WriteString(&sv_client->netchan.message, *s);
  }
  n = i - 1;
...
static void Cmd_PreSpawn_f(void) {
  ...
  if (buf >=
      sv.num_signon_buffers + sv.static_entity_count + sv.num_baseline_edicts)
    buf = 0;
  ...
  if (!buf) {
    ...
    if ((int)sv_mapcheck.value && check != sv.map_checksum &&
      ...
      SV_ClientPrintf(sv_client, PRINT_HIGH,
                      ...
                      "You may need a new version of the map, or the proper "
                      "install files.\n",
  ...
  if (buf < sv.static_entity_count) {
    ...
    while (buf < sv.static_entity_count) {
      ...
      if (sv_client->netchan.message.cursize >=
          (sv_client->netchan.message.maxsize / 2)) {
        break;
      }
    ...
    for (i = buf - sv.static_entity_count; i < sv.num_baseline_edicts; ++i) {
      ...
      if (!s->number || !s->modelindex ||
          !memcmp(s, &empty_baseline, sizeof(empty_baseline))) {
        ++buf;
        continue;
      }
      ...
      if (sv_client->fteprotocolextensions & FTE_PEXT_SPAWNSTATIC2) {
        ...
        SV_WriteDelta(sv_client, &empty_baseline, s,
        ...
        for (j = 0; j < 3; j++) {
          MSG_WriteCoord(&sv_client->netchan.message,
                         svent->e.baseline.origin[j]);
          MSG_WriteAngle(&sv_client->netchan.message,
                         svent->e.baseline.angles[j]);
        }
      }
      ++buf;
    }
    ...
    SZ_Write(&sv_client->netchan.message,
             sv.signon_buffers[buf - sv.static_entity_count -
                               sv.num_baseline_edicts],
             sv.signon_buffer_size[buf - sv.static_entity_count -
                                   sv.num_baseline_edicts]);
    ++buf;
  }
...
static void Cmd_Spawn_f(void) {
  ...
  for (i = n, client = svs.clients + n;
       i < MAX_CLIENTS && sv_client->netchan.message.cursize < (MAX_MSGLEN / 2);
       i++, client++)
  ...
  if (sv.loadgame) {
    ...
    sv_client->entgravity =
        fofs_gravity ? EdictFieldFloat(ent, fofs_gravity) : 1.0;
    sv_client->maxspeed = fofs_maxspeed ? EdictFieldFloat(ent, fofs_maxspeed)
                                        : (int)sv_maxspeed.value;
...
static void SV_SpawnSpectator(void) {
  ...
  sv_player->v->movetype =
      MOVETYPE_NOCLIP; // progs can change this to MOVETYPE_FLY, for example
...
static void Cmd_Begin_f(void) {
  ...
  if (!sv_client->spectator) {
    if (!*Info_Get(&sv_client->_userinfo_ctx_, "pmodel") ||
        !*Info_Get(&sv_client->_userinfo_ctx_,
                   "emodel")) // bliP: typo? 2nd pmodel to emodel
      SV_BroadcastPrintf(PRINT_HIGH,
                         "%s WARNING: missing player/eyes model checksum\n",
                         sv_client->name);
    else {
      pmodel = Q_atoi(Info_Get(&sv_client->_userinfo_ctx_, "pmodel"));
      emodel = Q_atoi(Info_Get(&sv_client->_userinfo_ctx_, "emodel"));

      if (!(pmodel == sv.model_newplayer_checksum ||
            pmodel == sv.model_player_checksum) ||
          emodel != sv.eyes_player_checksum)
        SV_BroadcastPrintf(
            PRINT_HIGH, "%s WARNING: non standard player/eyes model detected\n",
            sv_client->name);
    }
  }
...
static qbool SV_DownloadNextFile(void) {
  ...
  if (!(name = SV_MVDNum(num))) {
    Con_Printf((char *)Q_yelltext((unsigned char *)va(
        "Demo number %d not found.\n",
...
This is a sub routine for  SV_NextDownload(), called when download complete, we
set up some fields for sv_client.
...
#define CHUNKSIZE 1024
  ...
  if (chunknum <
      0) { // qqshka: FTE's chunked download does't have any way of signaling
           // what client complete dl-ing, so doing it this way.
    SV_CompleteDownoload();
    return;
  }
  ...
  if (sv_client->download_chunks_perframe) {
    ...
    if (sv_client->download_chunks_perframe >= maxchunks ||
        chunked_download_number < 1)
      return;
  }

  if (!sv_client->download_chunks_perframe) // ignore "rate" if not first packet
                                            // per frame
    if (sv_client->datagram.cursize + CHUNKSIZE + 5 + 50 >
        sv_client->datagram.maxsize)
      return; // choked!
  ...
  if (i > 0) {
    byte data[1 + (sizeof("\\chunk") - 1) + 4 + 1 + 4 +
              CHUNKSIZE]; // byte + (sizeof("\\chunk")-1) + long + byte + long +
                          // CHUNKSIZE
    ...
    if (sv_client->download_chunks_perframe) {
      ...
      MSG_WriteLong(msg, chunked_download_number); // return back, so they sure
                                                   // what it proper chunk
    } else
    ...
    if (sv_client->download_chunks_perframe)
      Netchan_OutOfBand(NS_SERVER, sv_client->netchan.remote_address,
                        msg->cursize, msg->data);
  } else {
...
static void Cmd_NextDownload_f(void) {
  ...
  r = (int)((curtime + frametime - sv_client->netchan.cleartime) /
            sv_client->netchan.rate);
  if (r <= 10)
    r = 10;
  ...
  Con_DPrintf(" => %d, total: %d => %d", r, sv_client->downloadsize,
              sv_client->downloadcount);
...
static void SV_NextUpload(void) {
  ...
  if (sv_client->upload) {
    ...
    if (pos == -1 || (sv_client->remote_snap &&
                      (pos + size) > (int)sv_maxuploadsize.value)) {
      msg_readcount += size;
      SV_CancelUpload();
      return;
    }
...
static void Cmd_Download_f(void) {
  ...
  if (
      // TODO: split name to pathname and filename and check for 'bad symbols'
      // only in pathname
      *name == '/'                // no absolute
      ...
      || ((i = strlen(name)) < 3
              ? 0
              : !strncmp(name + i - 3, "/..", 4)) // no /.. at end
      || *name == '.'                             // relative is pointless
      || ((i = strlen(name)) < 4
              ? 0
              : !strncasecmp(name + i - 4, ".log", 5)) // no logs
#ifdef _WIN32
      // no leading X:
      || (name[0] && name[1] == ':' &&
  ...
  if (sv_client->special)
    allow_dl = true; // NOTE: user used techlogin, allow dl anything in quake
                     // dir in such case!
  else if (!strstr(name, "/"))
    ...
    allow_dl = allow_download_sounds.value; // sounds
               ...
               name, "maps/",
               5)) // maps, note usage of allow_download_pakmaps a bit below
...
#define CLIENT_DOWNLOAD_RELATIVE_BASE                                          \
  FS_GAME // FIXME: Should we use FS_BASE ???
#else
#define CLIENT_DOWNLOAD_RELATIVE_BASE FS_BASE
  ...
  if (!sv_client->download && alternative_path[0]) {
    sv_client->download =
        FS_OpenVFS(alternative_path, "rb", CLIENT_DOWNLOAD_RELATIVE_BASE);
  }
  if (sv_client->download) {
  ...
  if (!sv_client->special) {
    ...
    if (sv_client->download && !strncmp(name, "maps/", 5) &&
        VFS_COPYPROTECTED(sv_client->download) &&
        !(int)allow_download_pakmaps.value) {
      SV_ClientDownloadComplete(sv_client);
      goto deny_download;
    }
  }
  ...
  sv_client->netchan.rate =
      1. / SV_BoundRate(true, Q_atoi(*val ? val : "99999"));
  ...
  if (!strncmp(name, "maps/", 5)) {
    SV_ClientPrintf(sv_client, PRINT_HIGH, "Map %s is %.0fKB (%.2fMB)\n", name,
                    (float)sv_client->downloadsize / 1024,
...
deny_download:
  ...
  if (sv_client->fteprotocolextensions & FTE_PEXT_CHUNKEDDOWNLOADS) {
    ...
    ClientReliableWrite_Long(
        sv_client,
        -1); // FIXME: -1 = Couldn't download, -2 = deny, we always use -1 atm
    ClientReliableWrite_String(sv_client, name);
  } else
#endif
  {
...
static void Cmd_DemoDownload_f(void) {
  ...
  unsigned char download_queue_already_exists[] =
      "Download queue already exists.\n";
  unsigned char cmdhelp_dldesc[] =
      "Download a demo from the server your are connected to";
  ...
  if (Cmd_Argc() < 2) {
    Con_Printf(
        ...
        Q_redtext(cmdhelp_dldesc), Q_redtext(cmdhelp_dl),
        Q_redtext(cmdhelp_pound), Q_redtext(cmdhelp_pound),
        Q_redtext(cmdhelp_pound), Q_redtext(cmdhelp_dl), Q_redtext(cmdhelp_dot),
        Q_redtext(cmdhelp_dot), Q_redtext(cmdhelp_dot), Q_redtext(cmdhelp_dot),
        Q_redtext(cmdhelp_dot), Q_redtext(cmdhelp_dot), Q_redtext(cmdhelp_dl),
        Q_redtext(cmdhelp_bs), Q_redtext(cmdhelp_stop),
        Q_redtext(cmdhelp_cancel));
    return;
  }
...
*/
static void Cmd_StopDownload_f(void) {
  unsigned char download_stopped[] =
      "Download stopped and download queue cleared.\n";
  ...
  if (sv_client->fteprotocolextensions & FTE_PEXT_CHUNKEDDOWNLOADS) {
    char *name =
        ""; // FIXME: FTE's chunked dl does't support "cmd stopdl", work around
...
static void SV_Say(qbool team) {
  ...
  char text[2048] = {
      0}; // used if mod does not have own support for say/say_team.
  qbool write_begin;
  ...
  if (p[0] == '"' && (j = (int)strlen(p)) > 2 && p[j - 1] == '"') {
    ...
    snprintf(text, sizeof(text), "%s",
             p + 1); // skip opening " and copy rest text including closing ".
  ...
  if (fp_messages) {
    ...
    if (tmp < 0)
      tmp = 10 + tmp;
    if (sv_client->whensaid[tmp] &&
        (curtime - sv_client->whensaid[tmp] < fp_persecond)) {
    ...
    if (team) {
      ...
      if (sv_client->spectator) {
        ...
        if (sv_client == client)
          ...
          if (!sv_sayteam_to_spec
                   .value // player can't say_team to spec in this case
              || !fake    // self say_team does't contain $\ so this is treat as
                          // private message
              || (client->spec_track <= 0 &&
                  strcmp(sv_client->team,
                         client->team)) // spec do not track player and on
                                        // different team
              || (client->spec_track > 0 &&
                  strcmp(sv_client->team,
                         svs.clients[client->spec_track - 1]
                             .team)) // spec track player on different team
          )
            continue; // on different teams
        } else if (coop.value)
  ...
  if (!team && ((sv_client->spectator && (int)sv_spectalk.value) ||
                !sv_client->spectator)) {
...
==================
*/
static void Cmd_Say_f(void) { SV_Say(false); }
/*
==================
Cmd_Say_Team_f
==================
*/
static void Cmd_Say_Team_f(void) { SV_Say(true); }
...
static void Cmd_Pings_f(void) {
  ...
  for (j = 0, client = svs.clients; j < MAX_CLIENTS; j++, client++) {
    if (!(client->state == cs_spawned ||
          (client->state ==
           cs_connected /* && client->spawncount != svs.spawncount*/)))
      continue;
...
static void Cmd_Upload_f(void) {
  ...
  if (!sv_client->uploadfn[0]) { // just in case..
...
char *shortinfotbl[] = {"name", "team", "skin", "topcolor", "bottomcolor",
...
static void Cmd_SetInfo_f(void) {
  ...
  if (sv_kickuserinfospamtime.value > 0 &&
      (int)sv_kickuserinfospamcount.value > 0) {
    ...
    } else if (++(sv_client->lastuserinfocount) >
               (int)sv_kickuserinfospamcount.value) {
      if (!sv_client->drop) {
  ...
  if (strstr(Cmd_Argv(1), "&c") || strstr(Cmd_Argv(1), "&r") ||
      strstr(Cmd_Argv(2), "&c") || strstr(Cmd_Argv(2), "&r"))
    return;

  strlcpy(oldval, Info_Get(&sv_client->_userinfo_ctx_, Cmd_Argv(1)),
          sizeof(oldval));
  ...
  if (!strcmp(Cmd_Argv(1), "name")) {
    ...
    //<-
    // VVD: forcenick ->
    // meag: removed sv_login check to allow optional logins... sv_forcenick
    // should still take effect
    if ((int)sv_forcenick.value &&
        /*(int)sv_login.value &&*/ sv_client->login[0]) {
      // allow differences in case, redtext
      if (Q_namecmp(sv_client->login, Cmd_Argv(2))) {
        SV_ClientPrintf(
            sv_client, PRINT_CHAT,
...
void ProcessUserInfoChange(client_t *sv_client, const char *key,
  ...
  if (mod_UserInfo_Changed) {
    ...
    PR_SetTmpString(&G_INT(OFS_PARM2),
                    Info_Get(&sv_client->_userinfo_ctx_, key));
...
==================
*/
static void Cmd_ShowServerinfo_f(void) { Info_Print(svs.info); }
...
static void Cmd_MinPing_f(void) {
  ...
  case 2:
    if (GameStarted())
      Con_Printf(
          "Can't change sv_minping: demo recording or match in progress.\n");
...
static void Cmd_AirStep_f(void) {
  ...
  case 2:
    if (GameStarted())
      Con_Printf("Can't change pm_airstep: demo recording in progress or "
                 "serverinfo key status is not 'Standby'.\n");
    else {
      ...
      if (val != 0 && val != 1)
        ...
        if (pm_airstep.value != old) { // seems value was changed
          SV_BroadcastPrintf(2, "%s turns %s %s\n", sv_client->name,
                             Q_redtext(red_airstep),
                             pm_airstep.value ? "on" : "off");
          break;
        }
      }
    }
...
static void Cmd_ShowMapsList_f(void) {
  ...
  Con_Printf(
      "Vote for maps by typing the mapname, for example \"%s\"\n\n---%s\n",
...
extern void CountPlayersSpecsVips(int *clients_ptr, int *spectators_ptr,
                                  int *vips_ptr, client_t **newcl_ptr);
...
static void Cmd_Join_f(void) {
  ...
  if (!(sv_client->extensions & Z_EXT_JOIN_OBSERVE)) {
    SV_ClientPrintf(sv_client, PRINT_HIGH,
                    "Your QW client doesn't support this command\n");
    return;
  }

  if (password.string[0] && strcmp(password.string, "none")) {
    SV_ClientPrintf(sv_client, PRINT_HIGH,
                    "This server requires a %s password. Please disconnect, "
                    "set the password and reconnect as %s.\n",
                    "player", "player");
    return;
  }
  ...
  if (!PlayerCanConnect(clients)) {
    SV_ClientPrintf(sv_client, PRINT_HIGH,
                    "Can't join, all player slots full\n");
    return;
  }
...
static void Cmd_Observe_f(void) {
  ...
  if (!SpectatorCanConnect(sv_client->vip, true /*kinda HACK*/, spectators,
                           vips)) {
    SV_ClientPrintf(sv_client, PRINT_HIGH,
                    "Can't join, all spectator/vip slots full\n");
    return;
  }
...
You're never quite sure if anyone might join the server and your team before you
finish saying a sentance. You run the risk of sounds around you being recorded
by quake, including but not limited to: TV channels, loved ones, phones, YouTube
videos featuring certain moans. Default on non-team games is to broadcast.
*/
...
void SV_VoiceReadPacket(void) {
  ...
  if (bytes > sizeof(ring->data) || curtime < host_client->lockedtill ||
      !sv_voip.ival) {
    MSG_ReadSkip(bytes);
    return;
  } else {
  ...
  if (sv.mvdrecording && sv_voip_record.ival &&
      !(sv_voip_record.ival == 2 && !host_client->spectator)) {
    // non-team messages should be seen always, even if not tracking any player
    if (vt == VT_ALL && (!host_client->spectator || sv_spectalk.ival)) {
      MVDWrite_Begin(dem_all, 0, ring->datalen + 6);
    } else {
      unsigned int cls;
      cls = ring->receiver[0] | (ring->receiver[1] << 8) |
            (ring->receiver[2] << 16) | (ring->receiver[3] << 24);
...
void SV_Voice_MuteAll_f(void) { host_client->voice_active = false; }

void SV_Voice_UnmuteAll_f(void) { host_client->voice_active = true; }

#endif // FTE_PEXT2_VOICECHAT

/*
 * Parse protocol extensions which supported by client.
 * This is workaround for the proxy case, like: qwfwd. We can't use it in case
 * of qizmo thought.
 */
  ...
  for (idx = 1; idx < Cmd_Argc();) {
    ...
    case PROTOCOL_VERSION_FTE:
      ...
      if (!sv_client->fteprotocolextensions) {
        sv_client->fteprotocolextensions =
            proto_value & svs.fteprotocolextensions;
        if (sv_client->fteprotocolextensions)
    ...
    case PROTOCOL_VERSION_FTE2:
      ...
      if (!sv_client->fteprotocolextensions2) {
        sv_client->fteprotocolextensions2 =
            proto_value & svs.fteprotocolextensions2;
        if (sv_client->fteprotocolextensions2)
    ...
    case PROTOCOL_VERSION_MVD1:
      if (!sv_client->mvdprotocolextensions1) {
        sv_client->mvdprotocolextensions1 =
            proto_value & svs.mvdprotocolextension1;
        if (sv_client->mvdprotocolextensions1)
...
void Cmd_Login_f(void) {
  ...
  if (sv_client->logged_in_via_web) {
    SV_ClientPrintf2(sv_client, PRINT_HIGH,
...
void Cmd_ChallengeResponse_f(void) {
  if (Cmd_Argc() != 2) {
    ...
    MSG_WriteString(&sv_client->netchan.message,
                    "Usage: challenge-response <response>\n");
    return;
  }
  ...
  Central_VerifyChallengeResponse(sv_client, sv_client->login_challenge,
                                  Cmd_Argv(1));
}
...
void Cmd_Logout_f(void) {
  ...
  if (sv_client->logged_in_via_web) {
    ...
    if (!(int)sv_login.value ||
        ((int)sv_login.value == 1 && sv_client->spectator)) {
      sv_client->logged = -1;
    }
  }
...
static ucmd_t ucmds[] = {
    ...
    {"pext", Cmd_PEXT_f,
     false}, // user reply with supported protocol extensions.
...
void SV_PreRunCmd(void) { memset(playertouch, 0, sizeof(playertouch)); }
...
void SV_RunCmd(usercmd_t *ucmd, qbool inside,
               qbool second_attempt) // bliP: 24/9
{
  int i, n;
  ...
  if (!inside && (int)sv_speedcheck.value
    ...
    /* AM101 method */
    tmp_time = Q_rint((realtime - sv_client->last_check) *
                      1000); // ie. Old 'timepassed'
    if (tmp_time) {
  ...
  // clamp view angles
  ucmd->angles[PITCH] =
      bound(sv_minpitch.value, ucmd->angles[PITCH], sv_maxpitch.value);
...
FIXME
  ...
  // Better would be to provide a way to simulate a move command, but at least
  // this doesn't require API change
  ...
  if (pr_nqprogs)
    sv_player->v->flags = ((int)sv_player->v->flags & ~FL_WATERJUMP) |
                          (pmove.waterjumptime ? FL_WATERJUMP : 0);
...
void SV_ServerSideWeaponRank(client_t *client, int *best_weapon,
                             int *best_impulse, int *hide_weapon,
                             int *hide_impulse) {
  ...
  *best_impulse = 0;
  ...
  for (i = 0; i < sizeof(client->weaponswitch_priority) /
                      sizeof(client->weaponswitch_priority[0]);
       ++i) {
...
static void SV_NotifyUserOfBestWeapon(client_t *sv_client, int new_impulse) {
  ...
  strlcpy(stuffcmd_buffer,
          va("//mvdsv_ssw %d %d\n", sv_client->weaponswitch_sequence_set,
             new_impulse),
          sizeof(stuffcmd_buffer));

  ClientReliableWrite_Begin(sv_client, svc_stufftext,
                            2 + strlen(stuffcmd_buffer));
  ClientReliableWrite_String(sv_client, stuffcmd_buffer);
}

static void SV_ExecuteServerSideWeaponForgetOrder(client_t *sv_client,
                                                  int best_impulse,
                                                  int hide_impulse) {
  char new_wrank[16] = {0};
  ...
  sv_client->weaponswitch_priority[1] =
      (hide_impulse == 1 || best_impulse == 2 ? 1 : 2);
  sv_client->weaponswitch_priority[2] =
      (sv_client->weaponswitch_priority[1] != 1 ? 1 : 0);
...
static void SV_ExecuteServerSideWeaponHideOnDeath(client_t *sv_client,
                                                  int hide_impulse,
                                                  int hide_weapon) {
  char new_wrank[16] = {0};

  if (sv_client->edict->v->health > 0.0f ||
      !sv_client->weaponswitch_hide_on_death) {
    return;
  }
  ...
  if (Info_Get(&sv_client->_userinfo_ctx_, "dev")[0] == '1' &&
      sv_client->edict->v->weapon != hide_weapon) {
    SV_ClientPrintf(sv_client, PRINT_HIGH, "Hiding on death: %d\n",
                    hide_impulse);
  }
}

static void SV_ServerSideWeaponLogic_PrePostThink(client_t *sv_client,
                                                  ssw_info_t *ssw) {
  ...
  if ((sv_client->mvdprotocolextensions1 & MVD_PEXT1_SERVERSIDEWEAPON) &&
      sv_client->weaponswitch_enabled) {
    int best_impulse, hide_impulse;
    qbool switch_to_best_weapon = false;
    int mode = sv_client->weaponswitch_mode;

    // modes: 0 immediately choose best, 1 preselect (wait until fire), 2
    // immediate if firing mode 2 = "preselect(1) when not holding +attack, else
    // immediate(0)"
    if (mode == 2) {
      mode = (ssw->firing ? 0 : 1);
    }
    switch_to_best_weapon = sv_client->weaponswitch_pending &&
                            (mode == 0 || ssw->firing) && (ent->health >= 1.0f);

    SV_ServerSideWeaponRank(sv_client, &ssw->best_weapon, &best_impulse,
                            &ssw->hide_weapon, &hide_impulse);

    ssw->hiding =
        (sv_client->weaponswitch_wasfiring && !ssw->firing && hide_impulse);
    sv_client->weaponswitch_wasfiring |= ssw->firing;

    if (switch_to_best_weapon && sv_client->weaponswitch_forgetorder) {
      SV_ExecuteServerSideWeaponForgetOrder(sv_client, best_impulse,
                                            hide_impulse);
    }
    SV_ExecuteServerSideWeaponHideOnDeath(sv_client, hide_impulse,
                                          ssw->hide_weapon);
...
static void SV_ServerSideWeaponLogic_PostPostThink(client_t *sv_client,
                                                   ssw_info_t *ssw) {
  entvars_t *ent = sv_client->edict->v;
  qbool dev_trace = (Info_Get(&sv_client->_userinfo_ctx_, "dev")[0] == '1');
  ...
  if (ssw->impulse_set) {
    qbool hide_failed =
        (ssw->impulse_set == 1 && ent->weapon != ssw->hide_weapon);
    qbool pickbest_failed =
        (ssw->impulse_set == 2 && ent->weapon != ssw->best_weapon);
...
// SV_RotateCmd
// Rotates client command so a high-ping player can better control direction as
// they exit teleporters on high-ping
void SV_RotateCmd(client_t *cl, usercmd_t *cmd_) {
  if (cl->lastteleport_teleport) {
    ...
    RotatePointAroundVector(result, up, direction,
                            cl->lastteleport_teleportyaw);
...
===================
*/
static void SV_ExecuteClientMove(client_t *cl, usercmd_t oldest,
                                 usercmd_t oldcmd, usercmd_t newcmd) {
  int net_drop;
...
#ifdef MVD_PEXT1_SERVERSIDEWEAPON
  {
    // This is necessary to interrupt LG/SNG where the firing takes place inside
    // animation frames
    if (sv_client->weaponswitch_enabled && sv_client->weaponswitch_pending &&
        !sv_client->edict->v->impulse) {
...
static void SV_DebugWriteServerAntilagPositions(client_t *cl, int present) {
  ...
  header.length = sizeof_mvdhidden_antilag_position_header_t +
                  present * sizeof_mvdhidden_antilag_position_t;
  ...
  if (MVDWrite_HiddenBlockBegin(sizeof_mvdhidden_block_header_t_range0 +
                                header.length)) {
    ...
    MVD_SZ_Write(&antilag_header.incoming_seq,
                 sizeof(antilag_header.incoming_seq));
    MVD_SZ_Write(&antilag_header.server_time,
                 sizeof(antilag_header.server_time));
    MVD_SZ_Write(&antilag_header.target_time,
                 sizeof(antilag_header.target_time));
...
static void SV_DebugWriteWeaponScript(byte playernum, qbool server_side,
                                      int items, byte shells, byte nails,
                                      byte rockets, byte cells, byte choice,
                                      const char *weaponlist) {
  if (sv_debug_weapons.value >= 1) {
    mvdhidden_block_header_t header;

    // Write out immediately
    header.type_id = (server_side ? mvdhidden_usercmd_weapons_ss
                                  : mvdhidden_usercmd_weapons);
...
static void SV_DebugClientSideWeaponScript(client_t *cl) {
  ...
  SV_DebugWriteWeaponScript(playernum, false, items, shells, nails, rockets,
                            cells, choice, weaponlist);
}
...
static void SV_DebugServerSideWeaponInstruction(client_t *cl) {
  if (sv_debug_weapons.value >= 1) {
    ...
    if (MVDWrite_HiddenBlockBegin(sizeof_mvdhidden_block_header_t_range0 +
      ...
      flags |=
          (cl->weaponswitch_hide == 1
               ? MVDHIDDEN_SSWEAPON_HIDE_AXE
               : (cl->weaponswitch_hide == 2 ? MVDHIDDEN_SSWEAPON_HIDE_SG : 0));
      flags |=
          (cl->weaponswitch_hide_on_death ? MVDHIDDEN_SSWEAPON_HIDEONDEATH : 0);
      flags |= (cl->weaponswitch_wasfiring ? MVDHIDDEN_SSWEAPON_WASFIRING : 0);
      flags |= (cl->weaponswitch_enabled ? MVDHIDDEN_SSWEAPON_ENABLED : 0);
      flags |=
          (cl->weaponswitch_forgetorder ? MVDHIDDEN_SSWEAPON_FORGETORDER : 0);

      memcpy(weaponlist, cl->weaponswitch_priority,
             min(10, sizeof(cl->weaponswitch_priority)));
...
static void SV_DebugServerSideWeaponScript(client_t *cl, int best_impulse) {
  if (sv_debug_weapons.value >= 1) {
    ...
    SV_DebugWriteWeaponScript(
        cl - svs.clients, true, ent->items, ent->ammo_shells, ent->ammo_nails,
        ent->ammo_rockets, ent->ammo_cells, best_impulse, encoded);
  }
}
#endif
...
#define MAX_EXTRAPOLATE 0.02
    ...
    if (sv_antilag_no_pred.value) {
      ...
      // try to figure out what time client is currently predicting, basically
      // this is just 6.5ms with 13ms ping and 13.5ms with higher might be off
      // with different max_physfps values
      target_time = min(frame->sv_time + (frame->ping_time < MAX_PREDICTION
                                              ? 1 / max_physfps
                                              : MAX_PREDICTION),
                        sv.time);
    }
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
      ...
      if (target_cl->state != cs_spawned ||
          target_cl->antilag_position_next == 0 ||
          (target_cl->spectator == 0 && target_cl->edict->v->health <= 0)) {
      ...
      // target player's movement commands are late, extrapolate his position
      // based on velocity
      if (target_time > target_cl->localtime) {
        VectorMA(target_cl->edict->v->origin,
                 min(target_time - target_cl->localtime, MAX_EXTRAPOLATE),
                 target_cl->edict->v->velocity, cl->laggedents[i].laggedpos);
        continue;
      }

      // we have only one antilagged position, use that
      if (target_cl->antilag_position_next == 1) {
        VectorCopy(target_cl->antilag_positions[0].origin,
                   cl->laggedents[i].laggedpos);
        continue;
      }

      // find the position before target time (base) and the one after that
      // (interpolate)
      for (j = target_cl->antilag_position_next - 2;
           j > target_cl->antilag_position_next - 1 - MAX_ANTILAG_POSITIONS &&
           j >= 0;
           j--) {
        if (target_cl->antilag_positions[j % MAX_ANTILAG_POSITIONS].localtime <
            target_time)
          break;
      }
      ...
      interpolate =
          &target_cl->antilag_positions[(j + 1) % MAX_ANTILAG_POSITIONS];

      // we have two positions, just interpolate between them
      factor = (target_time - base->localtime) /
               (interpolate->localtime - base->localtime);
      VectorInterpolate(base->origin, factor, interpolate->origin,
                        cl->laggedents[i].laggedpos);
    }

    cl->laggedents_count =
        MAX_CLIENTS; // FIXME: well, FTE do it a bit different way...
  ...
  while (1) {
    ...
    case clc_move:
      ...
      if (s[0]) {
        ...
        //				newcmd.forwardmove = newcmd.sidemove =
        // newcmd.upmove = 0;
        newcmd.buttons &=
            BUTTON_JUMP; // only jump button allowed while in console
      ...
      calculatedChecksum = COM_BlockSequenceCRCByte(
          ...
          MSG_GetReadCount() - checksumIndex - 1, seq_hash);
      ...
      if (cl->mvdprotocolextensions1 & MVD_PEXT1_HIGHLAGTELEPORT) {
        if (cl->lastteleport_outgoingseq &&
            cl->netchan.incoming_acknowledged < cl->lastteleport_outgoingseq) {
          if (cl->netchan.incoming_sequence - 2 >
              cl->lastteleport_incomingseq) {
            SV_RotateCmd(cl, &oldest);
          }
          if (cl->netchan.incoming_sequence - 1 >
              cl->lastteleport_incomingseq) {
      ...
      if (sv_antilag.value) {
        if (cl->antilag_position_next == 0 ||
            cl->antilag_positions[(cl->antilag_position_next - 1) %
                                  MAX_ANTILAG_POSITIONS]
                    .localtime < cl->localtime) {
          cl->antilag_positions[cl->antilag_position_next %
                                MAX_ANTILAG_POSITIONS]
              .localtime = cl->localtime;
          VectorCopy(cl->edict->v->origin,
                     cl->antilag_positions[cl->antilag_position_next %
                                           MAX_ANTILAG_POSITIONS]
                         .origin);
...
static qbool SV_ClientExtensionWeaponSwitch(client_t *cl) {
  ...
  if (write) {
    ...
    cl->weaponswitch_mode =
        (flags & (clc_mvd_weapon_mode_presel | clc_mvd_weapon_mode_iffiring));
...
static void SV_DebugClientCommand(byte playernum, const usercmd_t *usercmd,
                                  int dropnum_) {
  if (playernum >= MAX_CLIENTS) {
    return;
  }

  if (sv_debug_usercmd.value >= 1 ||
      svs.clients[playernum].mvd_write_usercmds) {
    mvdhidden_block_header_t header;
