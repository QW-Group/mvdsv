...
====================
*/
int SV_AreaEdicts(vec3_t mins, vec3_t maxs, edict_t **edicts, int max_edicts,
                  int area) {
  link_t *l, *start;
  edict_t *touch;
...
static void SV_TouchLinks(edict_t *ent, areanode_t *node) {
  ...
  numtouch = SV_AreaEdicts(ent->v->absmin, ent->v->absmax, touchlist,
                           sv.max_edicts, AREA_TRIGGERS);
...
void SV_LinkToLeafs(edict_t *ent) {
  ...
  ent->e.num_leafs = CM_FindTouchedLeafs(ent->v->absmin, ent->v->absmax,
...
int SV_PointContents(vec3_t p) {
  return CM_HullPointContents(&sv.worldmodel->hulls[0],
                              sv.worldmodel->hulls[0].firstclipnode, p);
}
...
edict_t *SV_TestEntityPosition(edict_t *ent) {
  ...
  if (ent->v->solid == SOLID_TRIGGER || ent->v->solid == SOLID_NOT)
    ...
    trace = SV_Trace(ent->v->origin, ent->v->mins, ent->v->maxs, ent->v->origin,
                     MOVE_NOMONSTERS, ent);
  else
    trace = SV_Trace(ent->v->origin, ent->v->mins, ent->v->maxs, ent->v->origin,
                     MOVE_NORMAL, ent);
...
==================
*/
trace_t SV_ClipMoveToEntity(edict_t *ent, vec3_t *eorg, vec3_t start,
...
void SV_ClipToLinks(areanode_t *node, moveclip_t *clip) {
  ...
  numtouch = SV_AreaEdicts(clip->boxmins, clip->boxmaxs, touchlist,
                           sv.max_edicts, AREA_SOLID);
  ...
  for (i = 0; i < numtouch; i++) {
    ...
    if (clip->type & MOVE_LAGGED) {
      // can't touch lagged ents - we do an explicit test for them later in
      // SV_AntilagClipCheck.
    ...
    if ((int)touch->v->flags & FL_MONSTER)
      trace = SV_ClipMoveToEntity(touch, NULL, clip->start, clip->mins2,
                                  clip->maxs2, clip->end);
    else
      trace = SV_ClipMoveToEntity(touch, NULL, clip->start, clip->mins,
                                  clip->maxs, clip->end);
...
void SV_AntilagClipCheck(areanode_t *node, moveclip_t *clip) {
  ...
  for (i = 0; i < w.maxlagents; i++) {
    ...
    if (touch == clip->passedict)
      continue;
    if (touch->v->solid == SOLID_TRIGGER)
      SV_Error("Trigger (%s) in clipping list",
               PR_GetEntityString(touch->v->classname));

    if ((clip->type & MOVE_NOMONSTERS) && touch->v->solid != SOLID_BSP)
      continue;

    VectorInterpolate(touch->v->origin, w.lagentsfrac, w.lagents[i].laggedpos,
                      lp);
    ...
    if ((int)touch->v->flags & FL_MONSTER)
      trace = SV_ClipMoveToEntity(touch, &lp, clip->start, clip->mins2,
                                  clip->maxs2, clip->end);
    else
      trace = SV_ClipMoveToEntity(touch, &lp, clip->start, clip->mins,
                                  clip->maxs, clip->end);
