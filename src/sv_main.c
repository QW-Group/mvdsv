...
qbool host_everything_loaded; // true if OnChange() applied to every var, end of
                              // Host_Init()

double curtime;  // not bounded or scaled, shared by local client and server.
double realtime; // affected by pause, you should not use it unless it something
                 // like physics and such.
...
char master_rcon_password[128] = ""; // bliP: password for remote server
                                     // commands
...
cvar_t sv_maxfps = {
    "maxfps", "77",
    CVAR_SERVERINFO}; // It actually should be called maxpps (max packets per
                      // second). It was serverinfo variable for quite long
                      // time, lets legolize it as cvar. Sad part is what we
                      // can't call it like sv_maxfps since clients relay on its
                      // name 'maxfps' already.

void OnChange_sysselecttimeout_var(cvar_t *var, char *value, qbool *cancel);
cvar_t sys_select_timeout = {"sys_select_timeout", "10000", 0,
                             OnChange_sysselecttimeout_var}; // microseconds.
...
cvar_t sv_hashpasswords = {"sv_hashpasswords",
                           "1"}; // 0 - plain passwords; 1 - hashed passwords
cvar_t sv_crypt_rcon = {
    "sv_crypt_rcon",
    "1"}; // use SHA1 for encryption of rcon_password and using timestamps
// Time in seconds during which in rcon command this encryption is valid (change
// only with master_rcon_password).
cvar_t sv_timestamplen = {"sv_timestamplen", "60"};
cvar_t sv_rconlim = {"sv_rconlim",
                     "10"}; // rcon bandwith limit: requests per second

// bliP: telnet log level
void OnChange_telnetloglevel_var(cvar_t *var, char *string, qbool *cancel);
cvar_t telnet_log_level = {"telnet_log_level", "0", 0,
                           OnChange_telnetloglevel_var};
//<-
...
cvar_t qconsole_log_say = {"qconsole_log_say", "0", 0,
                           OnChange_qconsolelogsay_var};
// logging "say" and "say_team" messages to the qconsole_PORT.log file

cvar_t sys_command_line = {"sys_command_line", "", CVAR_ROM};

cvar_t sv_use_dns = {
    "sv_use_dns", "0"}; // 1 - use DNS lookup in status command, 0 - don't use
cvar_t spectator_password = {"spectator_password",
                             ""}; // password for entering as a sepctator
cvar_t vip_password = {"vip_password",
                       ""}; // password for entering as a VIP sepctator
...
cvar_t sv_loadentfiles = {"sv_loadentfiles",
                          "1"}; // loads .ent files by default if there
cvar_t sv_loadentfiles_dir = {
    "sv_loadentfiles_dir",
    ""}; // check for .ent file in maps/sv_loadentfiles_dir first then just
         // maps/
cvar_t sv_default_name = {"sv_default_name", "unnamed"};

void sv_mod_msg_file_OnChange(cvar_t *cvar, char *value, qbool *cancel);
cvar_t sv_mod_msg_file = {"sv_mod_msg_file", "", CVAR_NONE,
                          sv_mod_msg_file_OnChange};
...
cvar_t samelevel = {"samelevel",
                    "1"};      // dont delete this variable - it used by mods
...
cvar_t sv_registrationinfo = {"sv_registrationinfo",
                              ""}; // text shown before "enter login"

// We need this cvar, because some mods didn't allow us to go at some placeses
// of, for example, start map.
...
qbool GameStarted(void) {
  ...
  for (d = demo.dest; d; d = d->nextdest)
    if (d->desttype !=
        DEST_STREAM) // oh, its not stream, treat as "game is started"
      break;
...
#define STATUS_SPECTATORS_AS_PLAYERS                                           \
  8 // for ASE - change only frags: show as "S"
  ...
  if (opt == STATUS_OLDSTYLE || (opt & (STATUS_PLAYERS | STATUS_SPECTATORS)))
    for (i = 0; i < MAX_CLIENTS; i++) {
      ...
      if ((cl->state >= cs_preconnected /* || cl->state == cs_spawned */) &&
          ((!cl->spectator &&
            ((opt & STATUS_PLAYERS) || opt == STATUS_OLDSTYLE)) ||
...
static void SVC_Log(void) {
  ...
  if (seq == svs.logsequence - 1 ||
      !logs[FRAG_LOG].sv_logfile) { // they already have this data, or we aren't
                                    // logging frags
    data[0] = A2A_NACK;
  ...
  strlcat(data, (char *)svs.log_buf[((svs.logsequence - 1) & 1)],
          MAX_DATAGRAM + 64);
...
static void SVC_GetChallenge(void) {
  ...
  // send it back
  snprintf(buf, sizeof(buf), "%c%i", S2C_CHALLENGE,
...
void FixMaxClientsCvars(void) {
  ...
  if ((int)maxspectators.value + maxclients.value + maxvip_spectators.value >
      MAX_CLIENTS)
    Cvar_SetValue(&maxvip_spectators, MAX_CLIENTS - (int)maxclients.value -
...
qbool CheckPasswords(char *userinfo, int userinfo_size, qbool *spass_ptr,
                     qbool *vip_ptr, int *spectator_ptr) {
  int spectator;
  qbool spass, vip;
...
void CountPlayersSpecsVips(int *clients_ptr, int *spectators_ptr, int *vips_ptr,
                           client_t **newcl_ptr) {
...
static void SVC_DirectConnect(void) {
  ...
  //
  // WARNING: WARNING: WARNING: using Cmd_TokenizeString() so do all Cmd_Argv()
  // above.
  //
  ...
  if ((spectator && !SpectatorCanConnect(vip, spass, spectators, vips)) ||
      (!spectator && !PlayerCanConnect(clients)) || !newcl) {
    ...
    if (spectator == 2 && !vip && vips < (int)maxvip_spectators.value) {
      vip = rip_vip = 1; // yet can be connected if realip is on vip list
               ...
               (((int)sv_forcespec_onfull.value == 2 &&
                 (Q_atoi(Info_ValueForKey(userinfo, "svf")) &
                  SVF_SPEC_ONFULL)) ||
  ...
  Netchan_Setup(NS_SERVER, &newcl->netchan, adr, qport,
                Q_atoi(Info_Get(&newcl->_userinfo_ctx_, "mtu")));
  ...
  // so demo.info[edictnum - 1].model == player_model so SV_MVDWritePackets()
  // will not wrote player model index, so client during playback this demo will
  // got invisible model, because model index will be 0. Fixing that. Btw,
  // struct demo contain different client specific structs, may be they need
  // clearing too, not sure. Also, we have Cmd_Join_f()/Cmd_Observe_f() which
  // have close behaviour to SVC_DirectConnect(), so I put same demo fix in
  // mentioned functions too.
 ...
 *
 *	If sv_rconlim.value is <= 0, the feature is disabled and false is
 *returned.
 *
 *	Note that the printing of the error message is delayed so we can
 *	properly print the rcon limit error rate that the system was trying to
 *do (i.e. 22000/100 rcon pps, etc...).  This can cause long delays in printing
...
static qbool rcon_bandlim(void) {
  ...
  if (realtime - lticks > 1.0) {
    if (lpackets > (int)sv_rconlim.value)
      Sys_Printf("WARNING: Limiting rcon response from %d to %d rcon pequests "
                 "per second from %s\n",
...
static void SVC_RemoteCommand(char *remote_command) {
  ...
  if (Rcon_Validate(remote_command, master_rcon_password)) {
    ...
    if (SV_FilterPacket()) // banned players can't use rcon, but we log it
    {
      ...
      for (i = 2; i < Cmd_Argc(); i++) {
        ...
        if (!strcasecmp(tstr, "rm") || !strcasecmp(tstr, "rmdir") ||
            !strcasecmp(tstr, "ls") || !strcasecmp(tstr, "chmod") ||
            !strcasecmp(tstr, "sv_admininfo") || !strcasecmp(tstr, "if") ||
  ...
  if (do_cmd) {
    ...
    if (plain[0])
      SV_Write_Log(RCON_LOG, 1,
                   va("Rcon from %s (%s): %s\n", NET_AdrToString(net_from),
                      plain, net_message.data + 4));
    else
      SV_Write_Log(RCON_LOG, 1,
                   va("Rcon from %s: %s\n", NET_AdrToString(net_from),
                      net_message.data + 4));
    ...
    if (!banned) {
      if (plain[0])
        SV_Write_Log(RCON_LOG, 1,
                     va("Bad rcon from %s (%s):\n%s\n",
                        NET_AdrToString(net_from), plain,
                        net_message.data + 4));
      else
        SV_Write_Log(RCON_LOG, 1,
                     va("Bad rcon from %s:\n%s\n", NET_AdrToString(net_from),
...
static void SV_AddIPVIP_f(void) {
  ...
  for (i = 0; i < numipvips; i++)
    if (ipvip[i].compare == 0xffffffff ||
        (ipvip[i].mask == f.mask && ipvip[i].compare == f.compare))
      break; // free spot
  if (i == numipvips) {
...
static void SV_RemoveIPVIP_f(void) {
  ...
  for (i = 0; i < numipvips; i++)
    if (ipvip[i].mask == f.mask && ipvip[i].compare == f.compare) {
...
static void SV_AddIP_f(void) {
  ...
  s = Cmd_Argv(3);
  if (long_time > 0) {
    if (*s == '+') // "addip 127.0.0.1 ban +10" will ban for 10 seconds from
                   // current time
      s++;
    else
      long_time = 0; // "addip 127.0.0.1 ban 1234567" will ban for some seconds
  ...
  for (i = 0; i < numipfilters; i++)
    if (ipfilters[i].compare == 0xffffffff ||
        (ipfilters[i].mask == f.mask && ipfilters[i].compare == f.compare))
      break; // free spot
  if (i == numipfilters) {
...
qbool SV_FilterPacket(void) {
  ...
  for (i = 0; i < numipfilters; i++)
    if (ipfilters[i].type == ipft_ban &&
        (in & ipfilters[i].mask) == ipfilters[i].compare)
...
void SV_BanList(void) {
  unsigned char blist[64] = "Ban list:", id[64] = "id", ipmask[64] = "ip mask",
                type[64] = "type", expire[64] = "expire";
  ...
  Con_Printf("%s\n"
             ...
             Q_redtext(blist), Q_redtext(id), Q_redtext(ipmask),
             Q_redtext(type), Q_redtext(expire));
...
void SV_Cmd_Ban_f(void) {
  ...
  // ============
  // get ADMIN rights from MOD via "mod_admin" field, mod MUST export such field
  // if wanna server ban support
  // ============
  ...
  switch (arg2c[0]) {
  case 's':
    break; // seconds is seconds
  case 'm':
    d *= 60;
    break; // 60 seconds per minute
  case 'h':
    d *= 60 * 60;
    break; // 3600 seconds per hour
  case 'd':
    d *= 60 * 60 * 24;
    break; // 86400 seconds per day
  default:
    Con_Printf("ban: wrong time arg\n");
    return;
  }
...
int SV_VIPbyPass(char *pass) {
  ...
  if (vip_values.string[0]) {
    use_value = true;
    // 2VVD: vip_password count may be not equal vip_values count, what we must
    // do in this case?
...
void SV_SavePenaltyFilter(client_t *cl, filtertype_t type, double pentime) {
  ...
  for (i = 0; i < numpenfilters; i++)
    if (SV_IPCompare(penfilters[i].ip, cl->realip.ip) &&
        penfilters[i].type == type) {
      return;
    }
...
double SV_RestorePenaltyFilter(client_t *cl, filtertype_t type) {
  ...
  for (i = 0; i < numpenfilters; i++) {
    if (type == penfilters[i].type &&
        SV_IPCompare(cl->realip.ip, penfilters[i].ip)) {
...
static void SV_ReadPackets(void) {
  ...
  for (i = 0, cl = svs.clients; i < MAX_CLIENTS; i++, cl++) {
    ...
    while (cl->packets &&
           (realtime - cl->packets->time >= cl->delay || sv.paused)) {
      SZ_Clear(&net_message);
...
static void SV_CheckTimeouts(void) {
  ...
  for (i = 0, cl = svs.clients; i < MAX_CLIENTS; i++, cl++) {
    ...
    if (cl->state == cs_zombie &&
        SV_ClientConnectedTime(cl) > zombietime.value) {
...
static void SV_CheckVars(void) {
  static char pw[MAX_KEY_STRING] = {0}, spw[MAX_KEY_STRING] = {0},
              vspw[MAX_KEY_STRING] = {0};
  ...
  if (strcmp(password.string, pw) || strcmp(spectator_password.string, spw) ||
      strcmp(vip_password.string, vspw)) {
  ...
  // check sv_maxrate
  if ((int)sv_maxrate.value != old_maxrate ||
      (int)sv_maxdownloadrate.value != old_maxdlrate) {
    client_t *cl;
    int i;
    char *val;
    ...
    for (i = 0, cl = svs.clients; i < MAX_CLIENTS; i++, cl++) {
      ...
      cl->netchan.rate = 1.0 / SV_BoundRate(cl->download != NULL,
                                            Q_atoi(*val ? val : "99999"));
    }
  }
}
...
void SV_InitLocal(void) {
  ...
  Info_SetValueForStarKey(svs.info, "*version", SERVER_NAME " " SERVER_VERSION,
                          MAX_SERVERINFO_STRING);
  Info_SetValueForStarKey(svs.info, "*z_ext", va("%i", SERVER_EXTENSIONS),
                          MAX_SERVERINFO_STRING);
  ...
  packet_freeblock = (packet_t *)Hunk_AllocName(
      MAX_DELAYED_PACKETS * sizeof(packet_t), "delayed_packets");

  for (i = 0; i < MAX_DELAYED_PACKETS; i++) {
    SZ_Init(&packet_freeblock[i].msg, packet_freeblock[i].buf,
            sizeof(packet_freeblock[i].buf));
...
void SV_ExtractFromUserinfo(client_t *cl, qbool namechanged) {
  ...
  if (namechanged) {
    ...
    for (p = val; *p; p++) {
      if ((*p & 127) == '\\' || *p == '\r' || *p == '\n' || *p == '$' ||
          *p == '#' || *p == '"' ||
          *p == ';') { // illegal characters in name, set some default
    ...
    if (p !=
        newname) // skip prefixed spaces, if any, even whole string of spaces
    ...
    if (!val[0] || !Q_namecmp(val, "console") || strstr(val, "&c") ||
        strstr(val, "&r")) {
  ...
  val = Info_Get(&cl->_userinfo_ctx_, cl->download ? "drate" : "rate");
  cl->netchan.rate =
      1.0 / SV_BoundRate(cl->download != NULL, Q_atoi(*val ? val : "99999"));

  // s2c packet dupes
  val = Info_Get(&cl->_userinfo_ctx_, "dupe");
  cl->dupe = atoi(val);
  cl->dupe = (int)bound(0, cl->dupe,
                        MAX_DUPLICATE_PACKETS); // 0=1 packet (aka: no dupes)
...
void OnChange_sysselecttimeout_var(cvar_t *var, char *value, qbool *cancel) {
  ...
  if (t < 1000 || t > 1000000) {
    Con_Printf("WARNING: sys_select_timeout can't be less then 1000 (1 "
               "millisecond) and more then 1 000 000 (1 second).\n");
    *cancel = true;
    return;
  }
}
...
void Host_Init(int argc, char **argv, int default_memsize) {
  ...
  // also apply that to variables which mirrored in userinfo because of
  // cl_parsefunchars was't applyed as this moment, same for serverinfo and may
  // be this fix something also.
...
void SV_LogPlayer(client_t *cl, char *msg, int level) {
  ...
  SV_Write_Log(PLAYER_LOG, level,
               va("%s\\%s\\%i\\%s\\%s\\%i%s\n", msg, name, cl->userid,
                  ...
                  cl->netchan.remote_address.port, info));
}
...
void SV_Write_Log(int sv_log, int level, char *msg) {
  ...
  case MOD_FRAG_LOG:
    ...
    error_msg =
        va("Can't write in %s log file: " /*%s/ */ "%sN.log.\n",
           /*fs_gamedir,*/ logs[sv_log].message_on, logs[sv_log].file_name);
    break;
  default:
