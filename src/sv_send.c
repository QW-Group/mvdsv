...
void SV_FlushRedirect(void) {
  ...
  if (sv_redirected == RD_PACKET) {
    ...
    NET_SendPacket(NS_SERVER, strlen(send1) + 1, send1, net_from);
             ...
             sv_redirectbufcount < MAX_REDIRECTMESSAGES) {
...
void SV_MulticastEx(vec3_t origin, int to, const char *cl_reliable_key) {
  ...
  for (j = 0, client = svs.clients; j < MAX_CLIENTS && !mvd_only;
       j++, client++) {
    int trackent = 0;
    ...
    if (fofs_trackent) {
      ...
      if (trackent < 1 || trackent > MAX_CLIENTS ||
          svs.clients[trackent - 1].state != cs_spawned)
        trackent = 0;
    }

    if (trackent) {
      VectorAdd(svs.clients[trackent - 1].edict->v->origin,
                svs.clients[trackent - 1].edict->v->view_ofs, vieworg);
    } else {
  ...
  inrange:
    if (reliable ||
        (cl_reliable_key && *cl_reliable_key &&
         strcmp("0", Info_Get(&client->_userinfo_ctx_, cl_reliable_key)))) {
  ...
  if (sv.mvdrecording) {
    if (mvd_only) {
      ...
      // write to dem_multiple(0), which will be skipped by all major clients
      // (ezQuake, FTE, fod)
      if (MVDWrite_HiddenBlockBegin(sv.multicast.cursize +
...
void SV_Multicast(vec3_t origin, int to) { SV_MulticastEx(origin, to, NULL); }
...
void SV_StartSound(edict_t *entity, int channel, char *sample, int volume,
  ...
  if (use_phs)
    SV_MulticastEx(origin, reliable ? MULTICAST_PHS_R : MULTICAST_PHS,
                   sv_reliable_sound.value ? "rsnd" : NULL);
  else
    SV_MulticastEx(origin, reliable ? MULTICAST_ALL_R : MULTICAST_ALL,
                   sv_reliable_sound.value ? "rsnd" : NULL);
}
...
void SV_WriteClientdataToMessage(client_t *client, sizebuf_t *msg) {
  ...
  if (ent->v->fixangle) {
    ...
    if (client->mvdprotocolextensions1 & MVD_PEXT1_HIGHLAGTELEPORT) {
      if (fofs_teleported) {
        client->lastteleport_teleport =
            ((eval_t *)((byte *)(client->edict)->v + fofs_teleported))->_int;
        ...
        client->lastteleport_teleportyaw =
            (client->edict)->v->angles[YAW] - client->lastcmd.angles[YAW];
  ...
  if (client->fteprotocolextensions &
      FTE_PEXT_ACCURATETIMINGS) { // the fte pext causes the server to send out
                                  // accurate timings, allowing for perfect
                                  // interpolation.
...
#endif
      if ((SERVER_EXTENSIONS & Z_EXT_SERVERTIME) &&
          (client->extensions &
           Z_EXT_SERVERTIME)) { // the zquake ext causes the server to send out
                                // peridoic timings, allowing for moderatly
                                // accurate game time.
...
void SV_UpdateClientStats(client_t *client) {
  ...
  if (fofs_trackent) {
    int trackent =
        ((eval_t *)((byte *)(client->edict)->v + fofs_trackent))->_int;
    if (trackent < 1 || trackent > MAX_CLIENTS ||
        svs.clients[trackent - 1].state != cs_spawned)
      trackent = 0;
  ...
  if (ent->v->health > 0 ||
      client->spectator) // viewheight for PF_DEAD & PF_GIB is hardwired
...
void SV_SendClientDatagram(client_t *client, int client_num) {
  ...
  {
          // we're sending fake invalid delta update, so that client won't
  update screen MSG_WriteByte (&msg, svc_deltapacketentities); MSG_WriteByte
  (&msg, 0); MSG_WriteShort (&msg, 0);
...
static void SV_UpdateToReliableMessages(void) {
  ...
  if (sv.mvdrecording)
    SZ_Write(&demo.datagram, sv.datagram.data,
             sv.datagram.cursize); // FIXME: ???
...
void SV_SendClientMessages(void) {
  ...
  if (fofs_visibility) {
    for (i = 0; i < MAX_CLIENTS; ++i) {
      ((eval_t *)((byte *)(svs.clients[i].edict)->v + fofs_visibility))->_int =
          0;
    }
  }
  ...
  for (i = 0, c = svs.clients; i < MAX_CLIENTS; i++, c++) {
    ...
    if (c->state == cs_spawned)
      ...
      Netchan_Transmit(&c->netchan, c->datagram.cursize,
                       c->datagram.data); // just update reliable
  ...
  if (c->edict->v->dmg_take || c->edict->v->dmg_save) {
    if (ent->v->dmg_take || ent->v->dmg_save) {
      ...
      if (MVDWrite_Begin(dem_single, i, length)) {
        ...
        for (i = 0; i < 3; i++)
          MVD_MSG_WriteCoord(other->v->origin[i] +
                             0.5 * (other->v->mins[i] + other->v->maxs[i]));
      }
...
void SV_SendDemoMessage(void) {
  ...
  demo.frames[demo.parsecount & UPDATE_MASK].pause_duration =
      (int)bound(0, (curtime - demo.curtime) * 1000.0f, 255);
