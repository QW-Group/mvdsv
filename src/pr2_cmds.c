...
intptr_t PF2_precache_vwep_model(char *s) {
  ...
  // the strings are transferred via the stufftext mechanism, hence the
  // stringency
...
=================
*/
void PF2_setsize(edict_t *e, float x1, float y1, float z1, float x2, float y2,
                 float z2) {
  // vec3_t min, max;
  e->v->mins[0] = x1;
  e->v->mins[1] = y1;
...
void PF2_centerprint(int entnum, char *s) {
  ...
  if ((int)sv_specprint.value & SPECPRINT_CENTERPRINT) {
    for (i = 0, spec = svs.clients; i < MAX_CLIENTS; i++, spec++) {
      ...
      if ((spec->spec_track == entnum) &&
          (cl->spec_print & SPECPRINT_CENTERPRINT)) {
...
=================
*/
void PF2_ambientsound(float x, float y, float z, char *samp, float vol,
                      float attenuation) {
  char **check;
  int i, soundnum;
  vec3_t pos;
...
=================
*/
void PF2_traceline(float v1_x, float v1_y, float v1_z, float v2_x, float v2_y,
                   float v2_z, int nomonsters, int entnum) {
  trace_t trace;
  vec3_t v1, v2;
  edict_t *ent;
...
=================
*/
void PF2_TraceCapsule(float v1_x, float v1_y, float v1_z, float v2_x,
                      float v2_y, float v2_z, int nomonsters, edict_t *ent,
...
void PF2_stuffcmd(int entnum, char *str, int flags) {
  ...
  if (strchr(buf, '\n')) {
    ...
    if (!(flags &
          STUFFCMD_IGNOREINDEMO)) // STUFFCMD_IGNOREINDEMO flag is NOT set
    {
      if (sv.mvdrecording) {
    ...
    if ((int)sv_specprint.value & SPECPRINT_STUFFCMD) {
      for (j = 0, spec = svs.clients; j < MAX_CLIENTS; j++, spec++) {
        ...
        if ((spec->spec_track == entnum) &&
            (cl->spec_print & SPECPRINT_STUFFCMD)) {
...
void PF2_executecmd(void) {
  int old_other,
      old_self; // mod_consolecmd will be executed, so we need to store this
...
static intptr_t PF2_FindRadius(int e, float *org, float rad) {
  ...
  for (e++; e < sv.num_edicts; e++) {
    ...
    for (j = 0; j < 3; j++)
      eorg[j] =
          org[j] - (ed->v->origin[j] + (ed->v->mins[j] + ed->v->maxs[j]) * 0.5);
...
void PF2_makestatic(edict_t *ent) {
  ...
  if (sv.static_entity_count >=
      sizeof(sv.static_entities) / sizeof(sv.static_entities[0])) {
    ED_Free(ent);
    return;
  }
...
void PF2_changelevel(char *s, char *entfile) {
  ...
  // this check is evil and cause lock on one map, if /map command fail for some
  // reason
...
void PF2_logfrag(int e1, int e2) {
  ...
  if (!tblock)
    s = va("%s\n", "#bad date#");
  else if ((int)frag_log_type.value) // need for old-style frag log file
...
//(int e1, char *key, char *valbuff, int sizebuff)
{
  ...
  if (e1 == 0) {
    if (key && key[0] == '\\') { // so we can check is server support such
                                 // "hacks" via infokey(world, "\\realip") f.e.
      key++;

      value = "no";

      if (!strcmp(key, "date_str") || !strcmp(key, "ip") ||
          !strncmp(key, "realip", 7) || !strncmp(key, "download", 9) ||
          !strcmp(key, "ping") || !strcmp(key, "*userid") ||
          !strncmp(key, "login", 6) || !strcmp(key, "*VIP") ||
          !strcmp(key, "*state"))
        value = "yes";
    } else if (!strcmp(key, "date_str")) { // qqshka - qvm does't have any time
                                           // builtin support, so add this
      date_t date;
    ...
    if (!strcmp(key, "ip"))
      ...
      snprintf(ov, sizeof(ov), "%d",
               cl->file_percent ? cl->file_percent : -1); // bliP: file percent
      ...
      value = cl->login;
    else if (!strcmp(key, "*VIP")) // qqshka: also located in userinfo, but this
                                   // is more safe/secure way, imo
      snprintf(ov, sizeof(ov), "%d", cl->vip);
    else if (!strcmp(key, "netname")) // qqshka: also located in userinfo, but
                                      // this is more safe/secure way, imo
      value = cl->name;
...
void PF2_fixme(void) { PR2_RunError("unimplemented bulitin"); }
...
intptr_t PF2_FS_GetFileList(char *path, char *ext, char *listbuff,
                            intptr_t buffsize, intptr_t flags) {
  //	extern	searchpath_t *com_searchpaths; // evil, because this must be
  // used in fs.c only...
  char *gpath = NULL;
...
int PF2_Add_Bot(char *name, int bottomcolor, int topcolor, char *skin) {
  ...
  snprintf(info, sizeof(info),
           ...
           "d\\emodel\\6967\\pmodel\\13845\\skin\\%s\\*bot\\1",
  ...
  SZ_InitEx(&newcl->netchan.message, newcl->netchan.message_buf,
            (int)sizeof(newcl->netchan.message_buf), true);
...
void PF2_SetBotUserInfo(int entnum, char *key, char *value, int flags) {
  ...
  if (gamedata.APIversion < 15)
    flags = 0;
  if (strstr(key, "&c") || strstr(key, "&r") || strstr(value, "&c") ||
      strstr(value, "&r"))
    return;
...
void PF2_SetBotCMD(int entnum, int msec, float a1, float a2, float a3,
                   int forwardmove, int sidemove, int upmove, int buttons,
                   int impulse) {
  client_t *cl;
...
void PF2_SetUserInfo(int entnum, char *k, char *v, int flags) {
  ...
  snprintf(s, sizeof(s), "PF2_SetUserInfo \"%-.*s\" \"%-.*s\"",
           (int)sizeof(key), k, (int)sizeof(value), v);

  Cmd_TokenizeString(s);

  // well, PR2_UserInfoChanged() may call PF2_SetUserInfo() again, so we better
  // save thouse
  ...
  for (i = 0; shortinfotbl[i] != NULL; i++) {
    if (!strcmp(key, shortinfotbl[i])) {
      ...
      // well, here we do not have if ( flags & SETUSERINFO_STAR ) because
      // shortinfotbl[] does't have any star key
...
intptr_t PR2_GameSystemCalls(intptr_t *args) {
  ...
  case G_Find:
    return PF2_Find(NUM_FOR_GAME_EDICT(VMA(1)), args[2] - pr_edict_offset,
                    VMA(3));
  case G_executecmd:
    PF2_executecmd();
    return 0;
  ...
  case G_SetBotUserInfo:
    ...
    return 0;
  case G_SetBotCMD:
    PF2_SetBotCMD(args[1], args[2], VMV(3), args[6], args[7], args[8], args[9],
                  args[10]);
    return 0;
  case G_QVMstrftime:
    VM_CheckBounds(sv_vm, args[1], args[2]);
