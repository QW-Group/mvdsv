...
// minimal cache which can be used for demos, must be few times greater than
// DEMO_FLUSH_CACHE_IF_LESS_THAN_THIS
...
//
// complete - just force flush for cached dests (dest->desttype ==
// DEST_BUFFEREDFILE)
//
void DestFlush(qbool complete) {
  ...
  for (d = demo.dest; d; d = d->nextdest) {
    ...
    case DEST_BUFFEREDFILE:
      if (d->cacheused + DEMO_FLUSH_CACHE_IF_LESS_THAN_THIS > d->maxcachesize ||
          complete) {
    ...
    case DEST_STREAM:
      if (d->io_time + qtv_streamtimeout.value <= Sys_DoubleTime()) {
        // problem what send() have internal buffer, so send() success some time
        // even peer side does't read, this may take some time before internal
        // buffer overflow and timeout trigger, depends of buffer size.
      ...
      if (d->cacheused && !d->error) {
        ...
        {
          //					d->error = true;
          // man says: The calls return the number of characters sent, or -1 if
          // an error occurred. so 0 is legal or what?
        ...
        } else { // error of some kind. would block or something
    ...
    {
      if ((unsigned int)sv_demoMaxSize.value &&
...
static int DestCloseAllFlush(qbool destroyfiles, qbool mvdonly) {
  ...
  while (d) {
    ...
    if (!mvdonly || d->desttype != DEST_STREAM) {
      ...
      *prev = d->nextdest;
      DestClose(d, destroyfiles); // NOTE: this free dest struck, so we can't
                                  // use 'd' below
      numclosed++;

      if (dt != DEST_STREAM && dest_name[0]) // ignore stream or empty file name
        Run_sv_demotxt_and_sv_onrecordfinish(dest_name, dest_path,
                                             destroyfiles);
    } else
      prev = &d->nextdest;
...
static qbool MVDWrite_BeginEx(byte type, int to, int size) {
  ...
  if (new_mvd_msg) {
    // we need add mvd header for next message since "type" or "to" differ from
    // previous message, or it first message in this frame.

    MVD_MSG_WriteByte(0); // 0 milliseconds

    demo.frames[demo.parsecount & UPDATE_MASK].lasttype = type;
    demo.frames[demo.parsecount & UPDATE_MASK].lastto = to;
    demo.frames[demo.parsecount & UPDATE_MASK].lastsize =
    ...
    case dem_stats:
      MVD_MSG_WriteByte(
          type | (to << 3)); // msg "type" and "to" incapsulated in one byte
      break;
    default:
    ...
    demo.frames[demo.parsecount & UPDATE_MASK].lastsize_offset =
        demo.frames[demo.parsecount & UPDATE_MASK]._buf_.cursize - 4;
  } else {
    // we must alredy have mvd header here of previous message, so just change
    // size in header
    byte *buf;
    int lastsize;
    ...
    buf = demo.frames[demo.parsecount & UPDATE_MASK]._buf_.data +
          demo.frames[demo.parsecount & UPDATE_MASK].lastsize_offset;
...
static void SV_MVDWritePausedTimeToStreams(demo_frame_t *frame) {
  // When writing out a paused frame, send a packet to let QTV keep delay in
  // sync
  if (frame->paused) {
    mvddest_t *d;
    sizebuf_t msg;
    ...
    MSG_WriteLong(&msg,
                  LittleLong(sizeof(short) +
                             sizeof(byte))); //  6-10: length = <short> + <byte>
    MSG_WriteShort(
        &msg,
        LittleShort(mvdhidden_paused_duration)); // 11-12: tell QTV how much
                                                 // time has really passed
    MSG_WriteByte(
        &msg, duration); //    13: true ms value, as demo packets will have 0
...
static qbool SV_MVDWritePacketsEx(int num) {
  ...
  for (; num; num--, demo.lastwritten++) {
    ...
    for (i = 0, cl = frame->clients, last_cl = demo.clients; i < MAX_CLIENTS;
         i++, cl++, last_cl++) {
      ...
      if (valid) {
        ...
        for (j = 0; j < 3; j++) {
          ...
          origin[j] =
              nextcl->origin[j] + f * (nextcl->origin[j] - cl->origin[j]);
        }
      } else {
    ...
    if (demo.frames[demo.lastwritten & UPDATE_MASK]._buf_.cursize) {
      ...
      DemoWrite(demo.frames[demo.lastwritten & UPDATE_MASK]._buf_.data,
                demo.frames[demo.lastwritten & UPDATE_MASK]._buf_.cursize);

      msec =
          0; // That matter, we wrote time mark, so next data(if any) in this
             // frame follow with zero milliseconds offset, since it same frame.
             // NOTE: demo.frames[demo.lastwritten&UPDATE_MASK]._buf_.cursize
             // possible to be zero, in this case we wrote msec below...
    }

    // Write data about players, if we have data.
    // also if we does't have data and did't wrote msec above, we wrote it here,
    // even packet will be empty, this will break fuh, but I think I correct,
    // also it doubtfull what we did't wrote msec above.
    if (msg.cursize || msec) {
...
static mvddest_t *SV_InitRecordFile(char *name) {
  ...
  if (!sv_silentrecord.value)
    SV_BroadcastPrintf(PRINT_CHAT, "Server starts recording (%s):\n%s\n",
                       (dst->desttype == DEST_BUFFEREDFILE) ? "memory" : "disk",
                       s + 1);
...
void SV_MVDStop(int reason, qbool mvdonly) {
  ...
  if (reason == 2 || reason == 3 || reason == 4) {
    if (reason == 4)
      mvdonly =
          false; // if serious error, then close all dests including qtv streams
    ...
    if (reason == 4)
      SV_BroadcastPrintf(PRINT_CHAT,
                         "Error in MVD/QTV recording, recording stopped\n");
    else if (reason == 3)
...
====================
*/
void SV_MVDStop_f(void) { SV_MVDStop(0, true); }
...
====================
*/
void SV_MVD_Cancel_f(void) { SV_MVDStop(2, true); }
...
// so demo.info[edictnum - 1].model == player_model so SV_MVDWritePackets() will
// not wrote player model index, so client during playback this demo will got
// invisible model, because model index will be 0. Fixing that. Btw, struct demo
// contain different client specific structs, may be they need clearing too, not
// sure.
...
qbool SV_MVD_Record(mvddest_t *dest, qbool mapchange) {
  ...
  if (!sv.mvdrecording) {
    ...
    // or we just use /record or whatever command first time and here no
    // recording yet
    ...
    for (i = 0; i < UPDATE_BACKUP; i++) {
      ...
      SZ_InitEx(&demo.frames[i]._buf_, demo.frames[i]._buf__data,
                sizeof(demo.frames[0]._buf__data), true);
    }

    // set up buffer for non releable data
    SZ_InitEx(&demo.datagram, demo.datagram_data, sizeof(demo.datagram_data),
              true);
  }
...
void SV_MVD_SendInitialGamestate(mvddest_t *dest) {
  ...
  sv.mvdrecording =
      true; // NOTE:  afaik set to false on map change, so restore it here
  ...
  // fix up extensions to match sv_bigcoords correctly. sorry for old clients
  // not working.
  if (msg_coordsize == 4)
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    ...
    flags = (DF_ORIGIN << 0) | (DF_ORIGIN << 1) | (DF_ORIGIN << 2) |
            ...
            DF_EFFECTS | DF_SKINNUM | (ent->v->health <= 0 ? DF_DEAD : 0) |
            (ent->v->mins[2] != -24 ? DF_GIB : 0) | DF_WEAPONFRAME | DF_MODEL;
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    ...
    stats[STAT_ITEMS] =
        (int)ent->v->items | ((int)PR_GLOBAL(serverflags) << 28);
...
void SV_MVD_Record_f(void) {
  ...
  strlcpy(newname,
          va("%s%s%s", sv_demoPrefix.string,
             SV_CleanName((unsigned char *)Cmd_Argv(1)), sv_demoSuffix.string),
          sizeof(newname) - 4);

  Sys_mkdir(va("%s/%s", fs_gamedir, sv_demoDir.string));

  snprintf(name, sizeof(name), "%s/%s/%s", fs_gamedir, sv_demoDir.string,
           newname);
...
void SV_MVDEasyRecord_f(void) {
  ...
  if (c == 2)
    ...
    if ((int)teamplay.value >= 1 && i > 2) {
      // Teamplay
      snprintf(name, sizeof(name), "%don%d_", Dem_CountTeamPlayers(Dem_Team(1)),
               Dem_CountTeamPlayers(Dem_Team(2)));
      if ((int)sv_demoExtraNames.value > 0) {
        strlcat(name,
                va("[%s]_%s_vs_[%s]_%s_%s", Dem_Team(1),
                   Dem_PlayerNameTeam(Dem_Team(1)), Dem_Team(2),
                   Dem_PlayerNameTeam(Dem_Team(2)), sv.mapname),
                sizeof(name));
      } else
        strlcat(name, va("%s_vs_%s_%s", Dem_Team(1), Dem_Team(2), sv.mapname),
                sizeof(name));
    } else {
      if (i == 2) {
        // Duel
        snprintf(name, sizeof(name), "duel_%s_vs_%s_%s", Dem_PlayerName(1),
                 Dem_PlayerName(2), sv.mapname);
      } else {
        // FFA
  ...
  strlcpy(name,
          va("%s%s%s", sv_demoPrefix.string,
             SV_CleanName((unsigned char *)name), sv_demoSuffix.string),
          MAX_DEMO_NAME);
  ...
  snprintf(name2, sizeof(name2), "%s",
           va("%s/%s/%s.mvd", fs_gamedir, sv_demoDir.string, name2));
  snprintf(name4, sizeof(name4), "%s",
           va("%s/%s.xml", sv_demoDir.string, name4));
...
void SV_MVDInit(void) {
  ...
  // but mods use it like that already, so we keep it for backward
  // compatibility.
