...
MAINDIR=/home/tony/zquake-source
...
ifeq ($(ARCH),axp)
TARGETS=$(BUILDDIR)/zqwsv 
#$(BUILDDIR)/qwcl.x11
else
TARGETS=$(BUILDDIR)/zqwsv $(BUILDDIR)/qwcl $(BUILDDIR)/qwcl.x11 $(BUILDDIR)/glqwcl $(BUILDDIR)/glqwcl.glx
endif
...
rpm-qwsv: qwsv.spec $(BUILD_RELEASE_DIR)/zqwsv $(QWPROGS)
...
tar-qwsv: $(BUILD_RELEASE_DIR)/zqwsv $(QWPROGS)
	if [ ! -d archives ];then mkdir archives;fi
	$(MAKE) copyfiles-qwsv DESTDIR=$(QWSV_RPMDIR)/$(INSTALLDIR)
...
copyfiles-qwsv:
	...
	-mkdirhier $(DESTDIR)/qw
	-mkdirhier $(DESTDIR)/qw/skins
	cp $(BUILD_RELEASE_DIR)/zqwsv $(DESTDIR)/.
	strip $(DESTDIR)/zqwsv
	chmod 755 $(DESTDIR)/zqwsv
...
qwsv.spec : $(MAINDIR)/qwsv.spec.sh $(BUILD_RELEASE_DIR)/zqwsv
	sh $< $(VERSION) $(RPM_RELEASE) $(INSTALLDIR) > $@
