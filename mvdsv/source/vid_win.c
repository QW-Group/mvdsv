	...
	// build the mode list, leaving room for the low-res stretched mode, if any
		...
		for (i = 0; m[i] != 0xFF; i++)
		{
				...
				(nummodes < MAX_MODE_LIST))
			{
				...
				snprintf (modelist[curmode].modedesc, VMODE_MODEDESC_LEN, "%dx%d", xRes, yRes);
...
void VID_InitMGLDIB (HINSTANCE hInstance)
{
	...
	strlcpy (modelist[0].modedesc, "320x240", VMODE_MODEDESC_LEN);
	modelist[0].mode13 = 0;
	...
	strlcpy (modelist[1].modedesc, "640x480", VMODE_MODEDESC_LEN);
	modelist[1].mode13 = 0;
	...
	strlcpy (modelist[2].modedesc, "800x600", VMODE_MODEDESC_LEN);
	modelist[2].mode13 = 0;
	...
	do
	{
			...
			(nummodes < MAX_MODE_LIST))
		{
					...
					DISP_CHANGE_SUCCESSFUL)
			{
				...
				snprintf (modelist[nummodes].modedesc, VMODE_MODEDESC_LEN, "%dx%d",
...
// don't bother if we have a real VGA mode 0x13 mode
	if (!is_mode0x13)
	{
		...
		if (cstretch > 0)
		{
			...
			snprintf (modelist[istretch].modedesc, VMODE_MODEDESC_LEN, "%dx%d",
...
void VID_CheckModedescFixup (int mode)
{
	...
	if (mode == MODE_SETTABLE_WINDOW)
	{
		...
		snprintf (modelist[mode].modedesc, VMODE_MODEDESC_LEN, "%dx%d", x, y);
